...
class FCFS:
    ...
    def invoke_Scheduler(self, process_info):
        ...
        for i in range(len(process_info)):
            s = sched.scheduler()
            event = s.enterabs(0, i, fcfs.nextTask,
    ...
    def schedulingAlgo(self, processInfo):
        ...
        for i in range(len(processInfo)):
            if initialTime < processInfo[i][1]:
                initialTime = processInfo[i][1]
            startTime.append(initialTime)
            initialTime = initialTime + processInfo[i][2]
            completionTime = initialTime
            endTime.append(completionTime)
            processInfo[i].append(completionTime)

        turnAroundTime = FCFS.calculateTurnaroundTime(self, processInfo)
        waitingTime = FCFS.calculateWaitingTime(self, processInfo)
        FCFS.printData(self, processInfo, turnAroundTime, waitingTime)
...
if __name__ == "__main__":
    ...
    for i in data['process_information']:
        ...
        processDetails.extend(
            [i['process_id'], i['arrival_time'], i['burst_time']])
        process_data.append(processDetails)
    fcfs = FCFS()
    fcfs.invoke_Scheduler(process_data)
    fcfs.schedulingAlgo(process_data)
    f.close()
